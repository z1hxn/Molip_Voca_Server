export const registerApiRoutes = (app, deps) => {
  ...
  app.get('/api/community/vocas', authOptional, async (_req, res) => {
    ...
    ;({ data: communityRows, error: communityError } = await adminSupabase
      .from('voca_sets')
      .select('*')
  ...
  app.get('/api/vocas/:id', authOptional, async (req, res) => {
    const access = await resolveVocaAccess(adminSupabase, req.params.id, req.user?.id || null)
    if (access.error) {
      sendError(res, access.error)
      return
    }
    if (!access.canRead) {
  ...
  app.get('/api/vocas/:id/role', authOptional, async (req, res) => {
    const access = await resolveVocaAccess(adminSupabase, req.params.id, req.user?.id || null)
    if (access.error) {
      sendError(res, access.error)
      return
  ...
  app.post('/api/study/answer', authOptional, async (req, res) => {
  ...
  app.post('/api/study/undo-answer', authOptional, async (req, res) => {
    const wordId = String(req.body?.wordId || '')
    const correct = Boolean(req.body?.correct)
  ...
  app.post('/api/study/mark', authOptional, async (req, res) => {
    const wordId = String(req.body?.wordId || '')
  ...
  app.post('/api/study/sessions', authOptional, async (req, res) => {
    if (!req.user) {
      res.status(201).json({ ok: true, guest: true })
      return
    }
  ...
  app.get('/api/study/progress', authOptional, async (req, res) => {
    if (!req.user) {
      res.json([])
      return
    }
  ...
  app.get('/api/study/sessions', authOptional, async (req, res) => {
    if (!req.user) {
      res.json([])
      return
    }
